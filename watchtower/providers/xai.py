...
class XAIChatCompletionsWrapper:
    """Wrapper for xAI chat completions using OpenAI-compatible SDK"""
    ...
    def create(self, *args, **kwargs):
        ...
        model = kwargs.get("model", args[1] if len(args) > 1 else "grok-beta")

        gateway_messages = convert_messages_to_gateway_format(messages)

        completion_request = build_completion_request(
            "xai", model, gateway_messages, self._wt, metadata, session_id
        )
