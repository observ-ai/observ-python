...
class OpenRouterChatCompletionsWrapper:
    """Wrapper for OpenRouter chat completions using OpenAI SDK with custom headers"""
    ...
    def create(self, *args, **kwargs):
        ...
        model = kwargs.get("model", args[1] if len(args) > 1 else "openai/gpt-4")

        gateway_messages = convert_messages_to_gateway_format(messages)

        completion_request = build_completion_request(
            "openrouter", model, gateway_messages, self._wt, metadata, session_id
        )
